...
// Transform HTML string, or array of strings, using defined transform.
...
exports.scrape = function(urls, opts, cb){
	...
	var scrape = function(url, done){
		var query = utils.extend({}, this.options.query, {url: url});
		request(query, function(err, res, body){
			var data = this.transform(body);
			done.call(this, err, data);
		}.bind(this));
	}
		...
		scrape.call(this, url, function(err, data){
			...
			// Otherwise, we're ready to return data
				...
				if(opts.merge){
					...
					if(this.options.sort) res.sort(this.options.sort);
				}
...
exports.each = function(arr, opts, cb){
	...
	var urls = arr.map(function(obj){
		// Use 'param' key in template if it's not an object.
		// Obviously arrays will pass as an object - but wouldn't
		// be useful anyway. Maybe implement querystring arrays?
		// Or indexes in template?
		var data = typeof obj === "object"
			? obj
			: {param: obj};

		return utils.template(opts.route, data);
	});
